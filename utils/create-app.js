...
`),
  ...
  try {
    ...
    console.log(
      `${chalk.cyan("Downloading files from repo . This might take a moment... \n")}`,
    );
    if (templateType === "full_stack") {
      await makeDir(STOREFRONT_DIR);
      console.log(
        `Creating a new Unchainedshop storefront template in ${chalk.green(STOREFRONT_DIR)}.`,
      );
      ...
      await retry(() => downloadAndExtractBackend(ENGINE_DIR), {
        retries: 3,
      });
