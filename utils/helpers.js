...
export const downloadAndExtractExample = (root, name) => {
  return pipeline(
    got.stream(
      "https://codeload.github.com/unchainedshop/unchained/tar.gz/master",
    ),
    tar.extract({ cwd: root, strip: 3 }, [`unchained-master/examples/${name}`]),
  );
};

export const downloadAndExtractBackend = (root) => {
  return pipeline(
    got.stream(
      "https://codeload.github.com/unchainedshop/unchained-app/tar.gz/main",
    ),
    tar.extract({ cwd: root, strip: 1 }, [`unchained-app-main`]),
  );
};
...
export const downloadAndExtractStorefront = (root) => {
  return pipeline(
    got.stream(
      `https://codeload.github.com/unchainedshop/unchained-storefront/tar.gz/main`,
    ),
    tar.extract({ cwd: root, strip: 1 }, [`unchained-storefront-main`]),
  );
};
